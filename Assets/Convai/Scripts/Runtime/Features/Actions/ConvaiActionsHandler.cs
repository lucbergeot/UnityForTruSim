...
private void ParseActions(string actionsString)
{
    ...
    if (!actionsString.StartsWith("$"))
    {
        string[] parts = actionsString.Split(' ');
        string target = parts[2]; // Get the target location
        
        ConvaiLogger.Info($"Processing move action to: {target}", ConvaiLogger.LogCategory.Actions);
        
        // Find the target and add it to action list
        GameObject targetObj = FindTargetObject(new[] { target });
        if (targetObj != null)
        {
            _actionList.Add(new ConvaiAction(ActionChoice.MoveTo, targetObj, null));
        }
        return;
    }
...
{
    // Skip 'say' commands
    if (action.Trim().StartsWith("say", StringComparison.OrdinalIgnoreCase))
    {
        ConvaiLogger.DebugLog($"Ignoring 'say' command during action parsing: {action}", ConvaiLogger.LogCategory.Actions);
        continue;
    }
    ...
    ParseSingleAction(actionWords);
        ...
        {
            if (actionWords[0].Trim().StartsWith("say", StringComparison.OrdinalIgnoreCase))
    {
        ConvaiLogger.DebugLog($"Ignoring 'say' command during single action parsing: {string.Join(" ", actionWords)}", ConvaiLogger.LogCategory.Actions);
        return;
    }
...
if (matchingAction == null || !string.Equals(matchingAction.action, actionString, StringComparison.OrdinalIgnoreCase))continue;
        ...
        private IEnumerator DoAction(ConvaiAction action)
        {
             if (!_actionsEnabled)
    {
        Debug.Log("Actions are disabled, skipping action execution.");
        yield break;
    }
            if (_currentNPC.IsCharacterTalking)
            {
             Debug.LogWarning("Cannot start action while NPC is talking");
             yield break;
            }

            isActionInProgress = true;

             float actionTimeout = 30f; // Timeout duration for actions
        ...
        public void ForceCompleteCurrentAction()
{
    try
    {
        Debug.Log("Force completing current action...");
        ...
        if (_currentNPC != null)
        {
            var animator = _currentNPC.GetComponent<Animator>();
            if (animator != null)
            {
        ...
        Debug.Log("Action forcefully completed");
    }
    catch (Exception e)
    {
        Debug.LogError($"Error during forced action completion: {e.Message}");
        isActionInProgress = false;  // Ensure this gets reset even if something fails
    }
}
        ...
        {
             if (animationName.Equals("Talk", StringComparison.OrdinalIgnoreCase))
            {
              _currentNPC.GetComponent<Animator>().SetBool("Talk", true);
              while (_currentNPC.IsCharacterTalking)
            {
              yield return null;
            }
             _currentNPC.GetComponent<Animator>().SetBool("Talk", false);
             yield break;
            }
       ...
       private IEnumerator MoveTowardsTarget(GameObject target, NavMeshAgent navMeshAgent)
{
    ...
    float moveTimeout = 30f; // Timeout duration for moving
    float startTime = Time.time;
    ...
    while (navMeshAgent.remainingDistance > navMeshAgent.stoppingDistance)
    {
        ...
        {
            ConvaiLogger.Warn("Move action timed out.", ConvaiLogger.LogCategory.Actions);
            break;
        }
